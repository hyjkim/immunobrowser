...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for i, r in enumerate(orm.Recombination.objects.all().iterator()):
            if r.n1_index <= 0:
                r.dj_index = r.j_index
            else:
                r.dj_index = r.n1_index

            if r.n2_index <= 0:
                r.vd_index = r.d_index
            else:
                r.vd_index = r.n2_index

            r.v_end = r.n2_index
            r.d_start = r.d_index + 1
            r.d_end = r.n1_index
            r.j_start = r.j_index + 1
            r.vd_insertion = r.n2_insertion
            r.dj_insertion = r.n1_insertion

            if r.v_ties is '':
                r.v_ties = None
            if r.j_ties is '':
                r.j_ties = None
            if r.v_gene_name is '(undefined)':
                r.v_gene_name = r.v_ties.split(", ")[0]
        ...
        start = datetime.datetime.now()
        for i, c in enumerate(orm.Clonotype.objects.all().iterator()):
            c.count = c.normalized_copy
    ...
    def backwards(self, orm):
        ...
        for r in orm.Recombination.objects.all():
            r.dj_index = 0
            r.vd_index = 0
            r.v_end = 0
            r.d_start = 0
        ...
        for c in orm.Clonotype.objects.all():
            c.count = c.normalized_copy
            c.frequency = c.normalized_frequency
            c.save()
