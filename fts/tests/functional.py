...
class ClonotypesTest(LiveServerTestCase):
    ...
    def DONTtest_can_add_clonotypes_from_tsv_file_from_admin_site(self):
        ...
        clonotypes_links = self.browser.find_elements_by_link_text(
            'Clonotypes')
    ...
    def DONTtest_navigate_to_the_repertoire_associated_with_a_sample(self):
    ...
    def DONTtest_can_view_all_clonotypes_from_summary_page(self):
        ...
        all_clonotype_link = self.browser.find_element_by_link_text(
            'View all Clonotypes')
        # He clicks the link and sees that there are only 2 total clonotypes for this dataset
        all_clonotype_link.click()

        clonotype_rows = self.browser.find_elements_by_xpath(
            '//table/tbody/tr')
        self.assertEquals(len(clonotype_rows), 2)

        self.fail('TODO')

    def DONTtest_clonotypes_all_view_shows_pages_of_clonotypes(self):
    ...
    def DONTtest_summary_figures_are_shown_on_summary_page(self):
    ...
    def DONTtest_can_compare_multiple_repertoires(self):
        # Bob navigates to the sample homepage
...
class SamplesTest(LiveServerTestCase):
    ...
    def DONTtest_view_all_samples_from_sample_site(self):
    ...
    def DONTtest_can_create_new_sample_via_admin_site(self):
        ...
        new_patient_links = self.browser.find_elements_by_link_text(
            "Jim Harbaugh")
...
class PatientsTest(LiveServerTestCase):
    ...
    def DONTtest_can_create_new_patient_via_admin_site(self):
        ...
        self.assertEqual(self.browser.current_url,
                         'http://localhost:8081/admin/patients/patient/')
        ...
        new_patient_link = self.browser.find_element_by_link_text(
            'Add patient')
        new_patient_link.click()
        ...
        self.assertEquals(
            'Freddy P. Soft Syndrome', disease_field.get_attribute('value'))
